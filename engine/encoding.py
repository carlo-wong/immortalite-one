...
def board_to_planes(board: chess.Board) -> np.ndarray:
    """Encode a board as a (20, 8, 8) float32 tensor (python-chess oracle path)."""
...
def fill_planes_from_fen(fen: str, out: np.ndarray) -> None:
    """Fill ``out`` (20, 8, 8) from a FEN string.

    Uses native ``fill_planes_from_fen`` when the extension is built; otherwise
    falls back to the python-chess path (oracle / evaluate / tests).
    """
    if _NATIVE is not None and hasattr(_NATIVE, "fill_planes_from_fen"):
        _NATIVE.fill_planes_from_fen(fen, out)
        return
    board = chess.Board(fen)
    _write_board_planes(board, out)


def fill_planes_batch_from_fens(fens: list[str], out: np.ndarray) -> None:
    """Fill ``out[i]`` from ``fens[i]``. Prefers native batch helper when present."""
    if _NATIVE is not None and hasattr(_NATIVE, "fill_planes_batch_from_fens"):
        _NATIVE.fill_planes_batch_from_fens(fens, out)
        return
    for i, fen in enumerate(fens):
        fill_planes_from_fen(fen, out[i])
...
def native_available() -> bool:
    """True when a native extension module was importable."""
