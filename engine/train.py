...
def _require_value_target_compat(
    expected: str,
    found: str | None,
    *,
    source: str,
    require_explicit_for_legacy: bool = False,
    explicit_cli: bool = False,
) -> None:
    """Reject silent value-target mixing between checkpoints/shards and cfg.

    Stamped artifacts must match ``expected``. Legacy artifacts (no stamp) are
    allowed only when ``require_explicit_for_legacy`` is False, or when the
    caller already supplied ``--value-target`` explicitly.
    """
    if found is None:
        if require_explicit_for_legacy and not explicit_cli:
            raise ValueError(
    ...
    try:
        with np.load(path) as data:
            if _shard_encoding_version(data) != ENCODING_VERSION:
                return []
            shard_vt = _shard_value_target(data)
            if expected_value_target is not None and shard_vt is not None:
                if shard_vt != expected_value_target:
                    print(
                        f"warning: skipping shard {os.path.basename(path)} "
                        f"(value_target={shard_vt} != {expected_value_target})"
                    )
                    return []
            planes = data["planes"]
...
def main() -> None:
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        ckpt_value_target: str | None = None
        if isinstance(state, dict):
            ckpt_encoding_version = int(state.get("encoding_version", 1))
            raw_vt = state.get("value_target")
            if raw_vt is not None:
                ckpt_value_target = str(raw_vt)
