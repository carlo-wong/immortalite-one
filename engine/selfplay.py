...
def _coerce_native_result(raw: Any, board: chess.Board, cfg: Config) -> SearchResult:
    ...
    return SearchResult(
        moves=moves,
        indices=list(raw["indices"] if isinstance(raw, dict) else raw.indices),
        visits=np.asarray(raw["visits"] if isinstance(raw, dict) else raw.visits, dtype=np.float64),
        q_values=np.asarray(
            raw["q_values"] if isinstance(raw, dict) else raw.q_values, dtype=np.float64
        ),
        priors=np.asarray(raw["priors"] if isinstance(raw, dict) else raw.priors, dtype=np.float64),
        clean_priors=np.asarray(
...
@dataclass
class _NativeGameState:
    board: chess.Board
    samples: list[Sample]
    moves: list[str]
    move_count: int
    low_value_streak: dict[chess.Color, int]
    last_root_value: float
...
def play_game(evaluator: NetEvaluator, cfg: Config, simulations: int,
    ...
    games = play_games_batched(
        evaluator, cfg, simulations, num_games=1, concurrency=1,
        tablebase=tablebase,
    )
    return games[0]


def _finalize_native_game(state: _NativeGameState, cfg: Config) -> GameResult:
    outcome = state.board.outcome(claim_draw=cfg.mcts.claim_draw)
    resigned = state.resigned_winner is not None
    tablebase_win = state.tablebase_winner is not None
    hit_max_moves = (
        state.move_count >= cfg.train.max_game_moves and outcome is None and not resigned
    )
    termination = _termination_reason(
        outcome,
        hit_max_moves=hit_max_moves,
        no_legal_moves=state.no_legal_moves,
        resigned=resigned,
        tablebase_win=tablebase_win,
        tablebase_draw=state.tablebase_draw,
    )
    winner_override = (
        state.tablebase_winner if state.tablebase_winner is not None else state.resigned_winner
    )
    _assign_values(
        state.samples,
        outcome,
        ...
        state.move_count,
        winner_override=winner_override,
        truncation_bootstrap=state.last_root_value,
    )
    if termination in {"checkmate", "resign", "tablebase_win"}:
        if winner_override is not None:
            winner = winner_override
...
def _native_start_session(
    native: Any, state: _NativeGameState, cfg: Config, simulations: int, add_noise: bool
) -> None:
    fen, history = _board_root_fen_and_moves(state.board)
    state.session = native.MctsSession(
        fen,
        simulations,
        _mcts_cfg_dict(cfg),
        add_noise,
...
def _native_apply_search_result(
    ...
    *,
    ...
    if not result.moves:
        state.no_legal_moves = True
        state.finished = _finalize_native_game(state, cfg)
        state.session = None
        return

    improved = result.improved_policy()
    state.last_root_value = float(result.searched_root_q)
    policy = np.zeros(POLICY_SIZE, dtype=np.float32)
    for idx, p in zip(result.indices, improved):
        policy[idx] = p
    state.samples.append(Sample(
        board_to_planes(state.board).astype(np.float16),
        policy.astype(np.float16),
        state.board.turn,
    ...
    resign_enabled = cfg.train.resign_plies > 0 and cfg.train.resign_threshold >= -1.0
    if resign_enabled and state.move_count >= cfg.train.resign_min_moves:
        player = state.board.turn
        if result.searched_root_q <= cfg.train.resign_threshold:
            state.low_value_streak[player] += 1
        else:
            state.low_value_streak[player] = 0
        if state.low_value_streak[player] >= cfg.train.resign_plies:
            state.resigned_winner = not player
            state.finished = _finalize_native_game(state, cfg)
            state.session = None
            return

    if state.move_count < exploration_moves:
        probs = improved.astype(np.float64)
        probs = probs / probs.sum()
        T = float(cfg.train.move_temperature)
        temp_plies = int(cfg.train.move_temperature_plies)
        if temp_plies > 0 and state.move_count < temp_plies and abs(T - 1.0) > 1e-12:
            probs = tempered_policy(probs, T)
        choice = np.random.choice(len(result.moves), p=probs)
        move = result.moves[choice]
    else:
...
def play_games_batched_native(
    ...
    *,
    ...
    active: list[_NativeGameState] = []
    launched = 0
    completed: list[GameResult] = []

    while len(completed) < num_games:
        while launched < num_games and len(active) < concurrency:
            active.append(_NativeGameState(
                board=chess.Board(),
                samples=[],
                moves=[],
                move_count=0,
        ...
        # Start searches / finish terminals for games without an active session.
        still_active: list[_NativeGameState] = []
        for state in active:
            if state.finished is not None:
                completed.append(state.finished)
                if on_game_finished is not None:
                    on_game_finished(state.finished)
                continue
        ...
        next_active: list[_NativeGameState] = []
        for state in active:
